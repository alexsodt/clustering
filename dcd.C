...
"*\n" );
	...
	for( int x = 0; x < nat ; x++ )
	{
		if( at[x].res != pres || strcasecmp( at[x].segid, psegid) )
			cres++;
		pres = at[x].res;
		strcpy( psegid, at[x].segid );
		...
		if( strlen(printBuffer) < 6 ) 
		{
                	for( int p = 0; p < 6 - strlen(printBuffer); p++ )
				fprintf(theFile, " ");
		}
...
void loadPSFfromPDB( FILE *theFile )
{
	...
	while( !feof(theFile) ) 
	{
		...
		if( !strncasecmp( buffer, "ATOM",4) || !strncasecmp( buffer, "HETA", 4) ) 
			local_nat++;
	}
...
{
	curHeader.header1.length = 'T';

	curHeader.header1.hdr_string[0] = '\0';
	curHeader.header1.hdr_string[1] = '\0';
	curHeader.header1.hdr_string[2] = '\0';
	curHeader.header1.hdr_string[3] = 'C';
	curHeader.header1.hdr_string[4] = 'O';
	curHeader.header1.hdr_string[5] = 'R';
	curHeader.header1.hdr_string[6] = 'D';

	curHeader.natom = nat;
	curHeader.header1.nfile=nframes;
	curHeader.header1.npriv=0;
	curHeader.header1.nsavc=1;
	curHeader.header1.nstep=nframes;
	curHeader.header1.unk1[0]=0;
	curHeader.header1.unk1[1]=0;
	curHeader.header1.unk1[2]=0;
	curHeader.header1.unk1[3]=0;
	curHeader.header1.fixed = 0;
	curHeader.header1.unk2[0] = 1065353216;
	curHeader.header1.qcrys = 1;
	curHeader.header1.dim4 = 0;
	curHeader.header1.qcg = 0;
	curHeader.header1.unk3[0]=0;	
	curHeader.header1.unk3[1]=0;	
	curHeader.header1.unk3[2]=0;	
	curHeader.header1.unk3[3]=0;	
	curHeader.header1.unk3[4]=0;	
	curHeader.header1.unk3[5]=0;	
	curHeader.header1.unk3[6]=24;	
	curHeader.header1.itemp =84;
	curHeader.header1.unk4[0]=164;
	curHeader.header1.ntitl=1;
	

	fwrite( &(curHeader.header1), sizeof(DCDheader1), 1, theFile );
	char title[80];
	for( int i = 0; i < 80; i++) title[i] = ' ';
	
	fwrite( title, sizeof(char), 80, theFile );
	
	int junk=164;
	fwrite( &junk, sizeof(int), 1, theFile );
	junk = 4;
	fwrite( &junk, sizeof(int), 1, theFile );
	fwrite( &nat, sizeof(int), 1, theFile );
	fwrite( &junk, sizeof(int), 1, theFile );
...
void writeFrame( FILE *theFile, struct atom_rec *at, double Lx, double Ly, double Lz )
{
	...
	if( curHeader.header1.qcrys )
	{
		...
		fwrite( &(a[0]), sizeof(double), 1, theFile );
		fwrite( &(a[1]), sizeof(double), 1, theFile );
		fwrite( &(b[1]), sizeof(double), 1, theFile );
		fwrite( &(a[2]), sizeof(double), 1, theFile );
		fwrite( &(b[2]), sizeof(double), 1, theFile );
		fwrite( &(c[2]), sizeof(double), 1, theFile );
	}
...
#define TRAJ_TYPE float
	...
	junk = 4 * natom;
	fwrite( &junk, sizeof(int), 1, theFile );
	fwrite( loc_x, sizeof(TRAJ_TYPE), natom, theFile );

	fwrite( &junk, sizeof(int), 1, theFile );
	fwrite( &junk, sizeof(int), 1, theFile );
	fwrite( loc_y, sizeof(TRAJ_TYPE), natom, theFile );

	fwrite( &junk, sizeof(int), 1, theFile );
	fwrite( &junk, sizeof(int), 1, theFile );
	fwrite( loc_z, sizeof(TRAJ_TYPE), natom, theFile );
...
{
	if( curHeader.natom != psf_natoms)
	{
		printf("Error: Header natoms (%d) doesn't match structure natoms (%d).\n",
			curHeader.natom, psf_natoms );
		exit(1);
	}
...
void loadPDB( FILE *theFile, struct atom_rec *at) 
{
		...
		if( !strncasecmp( buffer, "CRYST", 5 ) )
		{
			...
			free(at[a].resname);
			at[a].resname = (char *)malloc( sizeof(char) * (1 + strlen(res_name[a])) );
...
void copyDCDHeader( FILE *theFile1, FILE *theFile2, int activateQCRYS, int new_nframes )
{
...
//	curHeader.header1.nstep / curHeader.header1.nsavc
	...
	if( new_nframes >= 0 )
		curHeader.header1.nstep = curHeader.header1.nsavc * new_nframes;
