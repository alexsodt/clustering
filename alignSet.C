...
double alignStructuresOnAtomSet( double *xyz1_in, 
			...
			int *atoms_align2,
			int nat,
			int nat_tot, int sub_com, double *rotor_matrix  )
{	
...
//	printf("RMSD: %lf\n", (G_A + G_B - 2 * lam)/nat );
	...
	dsyev_(&jobz,&uplo,&order,K,&order,ev,&owork,&lwork,&info);
	double *work = (double *)malloc( sizeof(int)*(int)lround(owork));

	lwork = lround(owork);
	dsyev_(&jobz,&uplo,&order,K,&order,ev,work,&lwork,&info);
	...
	if( rotor_matrix )
	{
		rotor_matrix[0] = 1; rotor_matrix[1] = 0; rotor_matrix[2] = 0;
		rotor_matrix[3] = 0; rotor_matrix[4] = 1; rotor_matrix[5] = 0;
		rotor_matrix[6] = 0; rotor_matrix[7] = 0; rotor_matrix[8] = 1;

		double nvec[3] = {0,0,0};
		
		for( int v = 0; v < 3; v++ )
		{
			quat o_quat( 0, rotor_matrix+3*v );
			quat prod_quat = quat_imult( rot_quat, quat_mult( o_quat, rot_quat) );
			rotor_matrix[3*v+0] = prod_quat.x;
			rotor_matrix[3*v+1] = prod_quat.y;
			rotor_matrix[3*v+2] = prod_quat.z;
		}	
	}
...
//	printf("RMSD: %lf\n", (G_A + G_B - 2 * lam)/nat );
	...
	for( int i = 0; i < nat_tot; i++ )
	{
		...
		double dcom[3] = { cm1[0], cm1[1], cm1[2] };
		...
		xyz2_in[3*i+0] = ovec[0] + dcom[0];
		xyz2_in[3*i+1] = ovec[1] + dcom[1];
		xyz2_in[3*i+2] = ovec[2] + dcom[2];
#else
		xyz2_in[3*i+0] = prod_quat.x + dcom[0];
		xyz2_in[3*i+1] = prod_quat.y + dcom[1];
		xyz2_in[3*i+2] = ovec[2] + dcom[2];//prod_quat.z + cm1[2];
#endif
	}
